...
class ProfileRow extends Component {
  ...
  render() {
      ...
      transaction_item => {
        ...
        return (
          <tr>
            ...
            <td>
              {crypto.price * transaction_item.quantity} KWD
            </td>
          </tr>
        );
      }
    ...
    return (
      <>
        <tr>
          <th onClick={() => this.setState({ modal: true })} scope="row">
            {transaction.id}
          </th>
          <td>
        ...
        <Modal isOpen={this.state.modal} onClose={this.closeModal} style={customStyles}>
          <div>
              <div className="row">
                  <div className="col-12 text-right align-right">
                  <FontAwesomeIcon onClick={this.closeModal} icon={faWindowClose} color="red"size="1x" />
                  </div>
